...
export interface SriLankaGalleryItem {
  id: string;
  title: string;
  description: string;
  imageUrl: string;
...
/**
 * Reads Sri Lanka gallery items from localStorage (same key as admin panel),
 * sorted by the `order` field ascending.
 * Falls back to an empty array if nothing is stored yet.
 */
export function useSriLankaGallery(): SriLankaGalleryItem[] {
  try {
    const stored = localStorage.getItem(STORAGE_KEY);
    if (!stored) return [];
    const items: SriLankaGalleryItem[] = JSON.parse(stored);
