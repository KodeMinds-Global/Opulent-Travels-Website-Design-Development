...
import { useAdminMaldivesGalleryItem, useUpdateMaldivesGalleryItem } from '../../hooks/useAdminMaldivesGallery';
import { SriLankaGalleryItemForm, type SriLankaGalleryItemFormData } from '../../components/SriLankaGalleryItemForm';
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';

export function AdminMaldivesGalleryEditPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { data: item, isLoading } = useAdminMaldivesGalleryItem(id ?? '');
  const updateMutation = useUpdateMaldivesGalleryItem();
  ...
  const handleSubmit = async (data: SriLankaGalleryItemFormData) => {
    ...
    try {
      ...
      navigate('/admin/maldives-gallery');
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="sm"
          onClick={() => navigate('/admin/maldives-gallery')}
