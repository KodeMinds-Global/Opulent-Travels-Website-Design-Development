...
const SriLankaPackageDetail = () => {
  ...
  const [packageData, setPackageData] = useState<SriLankaPackage | null>(null);
  const [currentImageIndex, setCurrentImageIndex] = useState(0);
  
  useEffect(() => {
    if (packageId) {
      const foundPackage = getPackageById(packageId);
      if (foundPackage && foundPackage.type === 'sriLanka') {
        setPackageData(foundPackage as SriLankaPackage);
      } else {
        // Package not found or not a Sri Lanka package
  ...
  const packageImages = [
    ...
    ...packageData.locations.map(() => packageData.imageUrl)
  ];
            ...
            <div className="flex flex-wrap items-center gap-4 mb-8">
              ...
              <div className="flex items-center bg-white/20 dark:bg-white/10 backdrop-blur-sm px-3 py-1 rounded-full">
                ...
                <span className="text-white text-sm">
                  {packageData.locations.slice(0, 2).join(', ')}
                  {packageData.locations.length > 2 && ` +${packageData.locations.length - 2}`}
                </span>
              </div>
            </div>
